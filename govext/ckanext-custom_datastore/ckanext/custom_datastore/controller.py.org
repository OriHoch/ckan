...
class DatastoreController(ApiController):
    ...
    def apiCheck(self, environ, start_response):
        ...
        if not backFlag:
            if  not socketGetbyIP[0] in ServerAddress:
                if  remoteAddress not in ServerAddress:
                    ...
                    if config.get('ckan.api.open', 'NotInList').find(actionApi) < 0:
                        ...
                        logger.error(
                            "Access denied RemoteAddress = " + remoteAddress + " ServerAddress= " + ServerAddress + " API: " + actionApi + " socketGetbyIP= " + socketGetbyIP[0])
                        return self._finish(403, return_dict, content_type='json')
        else:
            setWho = 'Back'
            # aaa = self.action(self, actionApi,3)
        ...
        try:
            ...
            logger.debug("INapiCheck API: " + actionApi + "For sytem: " + setWho + e)
            return_dict['success'] = False
