...
import ckanext.custom_datastore.interfaces as interfaces
...
from ckanext.custom_datastore.backend import InvalidDataError
...
def insert_tracking(context, data_dict, api_name):
    ...
    try:
        userAgent = toolkit.request.environ['HTTP_USER_AGENT']
    except:
        remoteAddress = "No HTTP_USER_AGENT"
    try:
        urlapp = toolkit.request.application_url
    except:
        urlapp = " NO ORG URL"


    try:
        uidName = urlapp + " | " + context['auth_user_obj'].id + " | "+ context['auth_user_obj'].name
    except:
        uidName = urlapp
    ...
    try:
        # Insert into dataspusher db
        ##_execute_single_statement(context, sql_string,"")
        # Insert into ckan default db.
        eng = create_engine(config.get('sqlalchemy.url'))
        con = eng.connect()
        result = con.execute(sql_string)
        con.close()
    except sqlalchemy.exc.DataError:
        raise InvalidDataError(
            toolkit._("Problem with insert tracking"))
...
def search_data(context, data_dict):
    ...
    insert_tracking(context, data_dict ,'search_data')
