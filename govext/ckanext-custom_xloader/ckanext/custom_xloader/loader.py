...
def load_csv(csv_filepath, resource_id, mimetype='text/csv', logger=None):
    ...
    try:
        ...
        # logger cannot print in hebrew
        if fields and (fields[0]['id']) != 'he':
                ...
                with open(csv_filepath, 'rb') as f:
                    ...
                    try:
                        cur.copy_expert(
                            ...
                            .format(
                                resource_id=resource_id,
                                # support for hebrew headers
                                column_names=', '.join(['"{}"'.format(h.encode('UTF8'))
                                                        for h in headers]),
                                delimiter=delimiter,
                                encoding='UTF8',
                                ),
    ...
    # support for hebrew headers
    utf8_fields = [{'type': x['type'], 'id': x['id'].encode('UTF8')} for x in fields]
    _populate_fulltext(connection, resource_id, fields=utf8_fields)
...
def _populate_fulltext(connection, resource_id, fields):
    ...
    '''
    # support for hebrew headers (convert from unicode to str)
    sql = \
        '''
        UPDATE {table}
