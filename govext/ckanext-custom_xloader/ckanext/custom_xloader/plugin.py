...
from ckanext.custom_xloader import action, auth
import ckanext.custom_xloader.helpers as xloader_helpers
from ckanext.custom_xloader.loader import fulltext_function_exists, get_write_engine
...
class xloaderPlugin(plugins.SingletonPlugin):
    ...
    def update_config(self, config):
        ...
        p.toolkit.add_resource('fanstatic', 'custom_xloader_resources')

    # IConfigurable

    def configure(self, config_):
        if config_.get('ckanext.custom_xloader.ignore_hash') in ['True', 'TRUE', '1', True, 1]:
    ...
    def before_map(self, m):
        ...
        m.connect(
            'xloader_status', '/resource_status/{resource_id}',
            controller='ckanext.custom_xloader.controllers:ResourceDataController',
            action='xloader_status', ckan_icon='cloud-upload')
        return m
